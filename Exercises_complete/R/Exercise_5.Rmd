...
sir.forecast <- deSolve::ode(initial_state_values, 
                        seq(from = 10, to = 55, by = 1), 
                        sir_model, 
                        parms = fitval.forecast$par, )
sir.forecast <- data.table::as.data.table(sir.forecast)
```

Create a plot of your results
```{r}
p <- ggplot() + 
  # Original scenario fit to all data points
  geom_line(data = sir.fit, 
            mapping = aes(x = time, y = I), 
            color = 'red', size = 2 ) + 
  # Forecasted scenario
  geom_line(data = sir.forecast,
            mapping = aes(x = time, y = I), 
            color = 'blue', size = 2) + 
...
```{r}
fitval.forecast$par[["beta"]]/fitval.forecast$par[["gamma"]]
```
...
```{r}
fit.forecast.pars = cbind(
  beta = runif(100,fitval.forecast$par[['beta']]*.9, fitval.forecast$par[['beta']]*1.1),
  gamma = runif(100,fitval.forecast$par[['gamma']]*.9, fitval.forecast$par[['gamma']]*1.1)
)
fit.forecast.pars %>% head
```

Loop over each parameter combination: create a set of datapoints for each parameter combination
```{r}
for(i in 1:100){
  sir.forecast.sample <- deSolve::ode(initial_state_values, 
                        seq(from = 10, to = 55, by = 1), 
                        sir_model, 
                        parms = fit.forecast.pars[i,] )
  sir.forecast.sample <- data.table::as.data.table(sir.forecast.sample)
  sir.forecast.sample$run <- i
  if(i == 1){
    sir.forecast.uncertainty <-  sir.forecast.sample
    } else {
    sir.forecast.uncertainty <- rbind(sir.forecast.uncertainty, sir.forecast.sample)
  }
}
```
...
p <- ggplot() + 
  ...
  geom_line(data = sir.fit, 
            ...
            color = 'red', size = 2 ) + 
  # Forecasted scenario
  geom_line(data = sir.forecast.uncertainty,
            mapping = aes(x = time, y = I, group = run), 
            color = 'blue', size = .5) + 
  geom_point(data = plague.dat, mapping = aes(x = time, y = Deaths), size = 3) + 
  ylab("Deaths") + xlab("time (Weeks)") + 
  theme(axis.text = element_text(size = 20),
